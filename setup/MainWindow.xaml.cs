...
namespace ReShade.Setup
{
	public partial class MainWindow
	{
		...
		bool HasLocalReShadeInstallation(string basePath)
		{
			...
			if (File.Exists(Path.Combine(basePath, "ReShade.ini")))
			{
				return true;
			}

			foreach (string moduleName in new[] { "d3d9.dll", "d3d10.dll", "d3d11.dll", "d3d12.dll", "dxgi.dll", "opengl32.dll" })
			{
				if (GetModuleProductName(Path.Combine(basePath, moduleName)) == "ReShade")
				{
					return true;
				}
			}
		...
		void InstallStep_CheckExistingInstallation()
		{
			...
			if (!string.IsNullOrEmpty(selectedBasePath) && !selectedBasePath.Equals(targetBasePath, StringComparison.OrdinalIgnoreCase) && HasLocalReShadeInstallation(selectedBasePath))
			{
